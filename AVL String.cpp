...
AVLNode *deleteNode(AVLNode *root, string data)
{
    if (root == NULL)
        return root;
...
AVLNode *insertNode(AVLNode *root, string data)
{
    if (root == NULL)
    {
    ...
    //    cout << root->data << " " << data << " " << root->data.compare(data) << "\n";
        ...
        if (balanceTemp >= 2)
        {
            if ((root->left->data).compare(data) > 0)
                return rotateRight(root);
            else
            {
                root->left = rotateLeft(root->left);
        ...
        {
            if ((root->right->data).compare(data) > 0)
            {
                root->right = rotateRight(root->right);
                return rotateLeft(root);
            }
